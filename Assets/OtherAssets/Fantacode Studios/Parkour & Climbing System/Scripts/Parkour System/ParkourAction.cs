...
namespace FS_ParkourSystem
{
    ...
    public class ParkourAction : ScriptableObject
    {
        ...
        [SerializeField] bool useHands = true;
        ...
        public virtual bool CheckIfPossible(ObstacleHitData hitData, Transform player)
        {
            ...
            if (vaultType == VaultType.VaultOver && minimumObstacleHorizontalDistance == 0 && hitData.hasSpaceToVault)
                return false;
            ...
            var dir = hitData.heightHit.point - player.position;
            dir.y = 0;
            if (rotateToObstacle)
            ...
            {
                if (!hitData.ledgeHitFound || (hitData.ledgeHit.point - hitData.heightHit.point).magnitude > minimumObstacleHorizontalDistance)
                    return false;

                if (enableTargetMatching)
                    MatchPos = hitData.ledgeHit.point;
            }
    ...
    [CustomEditor(typeof(ParkourAction))]
    public class ParkourActionEditor : Editor
    {
        ...
        {
            base.OnInspectorGUI();
            var action = target as ParkourAction;
            GUILayout.Space(10);
            ...
            if (GUILayout.Button("Generate") || action.clip == null)
            {
                if (!string.IsNullOrEmpty(action.animName))
                {
                    ...
                    var parkourStateMachines = rootStateMachine.stateMachines.FirstOrDefault(s => s.stateMachine.name == "Parkour Actions");
                    ...
                    if (action.clip != null)
                    {
                        ...
                        if (state.state == null)
                        {
                            ...
                            addedState.position = new Vector3(pos.x, pos.y - 150, pos.z);
                        }
