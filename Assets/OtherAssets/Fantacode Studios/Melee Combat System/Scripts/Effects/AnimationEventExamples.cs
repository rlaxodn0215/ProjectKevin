...
namespace FS_CombatSystem
{
    public class AnimationEventExamples : MonoBehaviour
    {
        ...
        {
            if (!onlyForLastEnemy || (onlyForLastEnemy && obj.GetComponent<FighterCore>().TargetsInRange.Count == 1))
            {
        ...
        public void PlaySfx(GameObject obj, AudioClip audioClip)
        {
            FSAudioUtil.PlaySfxNew(audioClip);
        }
    }
}