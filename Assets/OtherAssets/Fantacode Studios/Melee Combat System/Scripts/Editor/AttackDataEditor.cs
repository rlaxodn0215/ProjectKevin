...
namespace FS_CombatSystem
{
    ...
    public class AttackDataEditor : AnimationPreviewHandler
    {
        ...
        public SerializedProperty rotateToTargetSettings;
        public SerializedProperty rotateToTargetType;
        public SerializedProperty rotateToTargetOffset;
        public SerializedProperty rotateToTargetStartTime;
        public SerializedProperty rotateToTargetEndTime;
        ...
        // For Rotate To Target
        static float startValueR;
        static float endValueR = 1;
        ...
        public override void OnEnable()
        {
            ...
            var blockedReaction = serializedObject.FindProperty("blockedReaction");
            blockedReactionClipInfo = blockedReaction.FindPropertyRelative("animationClipInfo");
            ...
            rotateTargetInBlocked = blockedReaction.FindPropertyRelative("rotateToAttacker");
            rotationInBlocked = blockedReaction.FindPropertyRelative("rotationOffset");
            ...
            rotateToTargetSettings = serializedObject.FindProperty("rotateToTargetSettings");
            rotateToTargetType = rotateToTargetSettings.FindPropertyRelative("rotationType");
            rotateToTargetStartTime = rotateToTargetSettings.FindPropertyRelative("rotateStartTime");
            rotateToTargetEndTime = rotateToTargetSettings.FindPropertyRelative("rotateEndTime");
            rotateToTargetOffset = rotateToTargetSettings.FindPropertyRelative("rotationOffset");
            ...
            if (attackData.Clip.clip != null)
            {
                ...
                startValueR = attackData.RotateStartTime;
                endValueR = attackData.RotateEndTime;
            }
            else
            {
                startValueI = 0;
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (rotateToTarget.boolValue)
            {
                EditorGUILayout.PropertyField(rotateToTargetType);
                EditorGUILayout.PropertyField(rotateToTargetOffset, new GUIContent("Rotation Offset (Angles)"));

                if (rotateToTargetType.enumValueIndex == (int)RotationType.Linear)
                {
                    GUILayout.Label("Rotation Time");
                    var sr = startValueR;
                    var er = endValueR;

                    GUILayout.BeginHorizontal();
                    EditorGUI.BeginChangeCheck();
                    EditorGUILayout.PropertyField(rotateToTargetStartTime, GUIContent.none, GUILayout.Width(40));
                    if (EditorGUI.EndChangeCheck())
                        rotateToTargetStartTime.floatValue = Mathf.Clamp01(rotateToTargetStartTime.floatValue);

                    EditorGUI.BeginChangeCheck();
                    EditorGUILayout.MinMaxSlider(ref startValueR, ref endValueR, 0, 1);
                    if (EditorGUI.EndChangeCheck())
                    {
                        var time = -1f;
                        if (sr != startValueR)
                            time = startValueR;
                        else if (er != endValueR)
                            time = endValueR;
                        if (time > 0)
                        {
                            ChangeAnimationClip(PreviewAnimation.Attack);
                            if (clip != null)
                                previewTime = time * clip.length;
                            rotateToTargetStartTime.floatValue = (float)Math.Round(startValueR, 2);
                            rotateToTargetEndTime.floatValue = (float)Math.Round(endValueR, 2);
                            UpdatePreview();
                        }
                    }

                    EditorGUI.BeginChangeCheck();
                    EditorGUILayout.PropertyField(rotateToTargetEndTime, GUIContent.none, GUILayout.Width(40));
                    if (EditorGUI.EndChangeCheck())
                        rotateToTargetEndTime.floatValue = Mathf.Clamp01(rotateToTargetEndTime.floatValue);
                    startValueR = rotateToTargetStartTime.floatValue;
                    endValueR = rotateToTargetEndTime.floatValue;
